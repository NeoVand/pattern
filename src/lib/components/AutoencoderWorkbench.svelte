<script lang="ts">
	...
	import type { AutoencoderDataset, AutoencoderData } from '$lib/ml/autoencoder-datasets';
	...
	let { source }: { source: AutoencoderDataset } = $props();
	const uid = $props.id();
	let dataset = $state.raw<AutoencoderData | null>(null);
	...
	const className = (label: number) => source.labels[label];
	const description = (label: number) =>
		source.id === 'mnist' ? `handwritten ${label}` : className(label).toLowerCase();
	const heading = $derived(
		mode === 'image'
			...
			: mode === 'morph'
				? `Between two ${source.id === 'mnist' ? 'digits' : 'items'}`
				: `Near ${description(selectedLabel)}`
	);
	...
	async function initialize() {
		...
		try {
			...
			loadNotice = '';
			try {
				const response = await fetch(asset(source.checkpoint), { signal: controller.signal });
			...
			dataset = data;
			saved = false;
			const a = data.testY.findIndex((label) => label === source.initialPair[0]),
				b = data.testY.findIndex((label) => label === source.initialPair[1]);
			pair = [a, b];
			selected = a;
	...
	function browse(delta: number) {
		if (!dataset) return;
		select((referenceIndex + delta + dataset.testY.length) % dataset.testY.length);
	}
...
<section
	class="ssl-lab"
	aria-label={`${source.name} autoencoder workbench`}
	data-dataset={source.id}
	{@attach mount}
>
	<header class="ssl-header">
		<div>
			<span class="ssl-kicker">{source.kicker}</span>
			<h3>A world inside two numbers.</h3>
			<p>
				Start with random weights. Watch 2,000 unseen {source.items} find their places as the model learns.
			</p>
		</div>
	...
	<div class="ssl-toolbar">
		...
		<span class="ssl-status" role="status" data-batch-ms={ms} data-backend={device}
			><i class:running></i>{phase === 'loading'
				? `Preparing ${source.name} and the training backend…`
				: phase === 'resetting'
	...
	{#if phase === 'error'}<div class="ssl-error" role="alert">
			<p>{error}</p>
			<button onclick={() => initialize()}>Load {source.name} again</button>
		...
		<div class="ssl-scores" class:has-history={history.length > 1}>
			...
			<div>
				...
				><small>{dataset.testY.length.toLocaleString('en-US')} official test examples</small>
			</div>
			{#if history.length > 1}
		...
		<div class="ssl-workspace">
			<div class="ssl-map-panel">
				...
				<div
					class="ssl-digit-key"
					aria-label={source.id === 'mnist' ? 'Color by digit' : 'Color by clothing category'}
				>
					...
					>{#each DIGIT_COLORS as color, digit (digit)}<button
							...
							aria-label={`Highlight ${source.id === 'mnist' ? `digit ${digit}` : className(digit)}`}
							aria-pressed={filter === digit}
							onclick={() => (filter = filter === digit ? -1 : digit)}>{className(digit)}</button
						>{/each}<span>Color reveals labels. Learning never uses them.</span>
				</div>
				<div class="ssl-morph">
					<button
						class="ssl-endpoint"
						aria-label={`Select first morph ${source.item}`}
						onclick={() => {
							mode = 'morph';
							mix = 0;
						}}
					...
					><button
						class="ssl-endpoint"
						aria-label={`Select second morph ${source.item}`}
						onclick={() => {
							mode = 'morph';
							mix = 1;
						}}
						><MnistDigit
							...
							label={`Second morph ${source.item}`}
			...
			<aside class="ssl-inspector" aria-label={`Inspect the ${source.item} and decoder`}>
				<div class="ssl-inspector-heading">
					<span>{heading}</span>
					<div>
						<button aria-label={`Previous ${source.name} ${source.item}`} onclick={() => browse(-1)}
							><PatternIcon name="arrowLeft" size={15} /></button
						><button aria-label={`Next ${source.name} ${source.item}`} onclick={() => browse(1)}
				...
				<div class="ssl-roundtrip">
					<div>
						<MnistDigit
							...
							label={`${mode === 'image' ? 'Original' : 'Nearest real'} ${description(selectedLabel)}, test image ${referenceIndex + 1}`}
							color={DIGIT_COLORS[selectedLabel]}
						/><span>{mode === 'image' ? 'Original' : `Nearest real ${source.item}`}</span>
					</div>
				...
				<p class="ssl-inspector-note">
					{mode === 'image'
						...
						: mode === 'probe'
							? `Drag to decode any coordinate. The reference follows the nearest real ${source.item}; it is not the source of this free point.`
				...
				<div class="ssl-neighbors">
					<span>{source.id === 'mnist' ? 'Nearby handwriting' : 'Nearby clothing'}</span>
					<div>
						{#each neighbors as neighbor (neighbor.index)}{@const index = neighbor.index}<button
								aria-label={`Inspect test image ${index + 1}, ${description(dataset.testY[index])}`}
								onclick={() => select(index)}
								><MnistDigit
									pixels={imageAt(index)}
									label={`Held-out ${description(dataset.testY[index])}`}
		...
		<details class="ssl-method">
			...
			<p>
				...
				Training samples that Gaussian; the map shows each image’s exact two means.
			</p>
			<p>
				...
				actual coordinates move. All {dataset.testY.length.toLocaleString('en-US')} official test images
				are drawn, and none update weights. Labels only assign color. The two-number bottleneck loses
				detail, so reconstructions can blur. Display pixels are clipped to 0–1; errors use the raw linear
				output.
			</p>
			<p>
				The {source.name} saved example was trained for 10,000 updates on its {dataset.trainY.length.toLocaleString(
					'en-US'
				)} training images, without labels. Choose random weights to start a new model. Live learning
				continues until you pause, switch datasets, or leave the workbench, using a dedicated browser
				worker with JaxJS’s compiled WebGPU kernels when available and WebAssembly otherwise. The decoder
				runs locally so dragging never waits for training.
				<a href={asset(source.provenance)} target="_blank" rel="noreferrer"
					>Dataset and model provenance</a
				>.
			</p>
		</details>
