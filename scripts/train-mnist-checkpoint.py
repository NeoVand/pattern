"""Reproduce Pattern's GELU variational MNIST or Fashion-MNIST starting point.
Same reparameterized Gaussian waist/MSE+KL objective as the JaxJS worker.
No labels are read. Requires NumPy and Pillow; run with [steps=10000] [--dataset fashion].
"""
...
meta=json.loads((DATA/f'{args.dataset}-meta.json').read_text())
...
x=images(f'{args.dataset}-train.png',meta['train']);test=images(f'{args.dataset}-test.png',meta['test']);rng=np.random.default_rng(7)
w=[];b=[]
for i in range(6):
...
def save(step,started):
    packed=np.concatenate([a.ravel() for pair in zip(w,b) for a in pair]).astype('<f4');packed.tofile(DATA/f'{args.dataset}-autoencoder.f32')
    z=forward(test)[0][3]
    info={'model':'variational autoencoder','architecture':SHAPE,'latentHead':'2 means + 2 log variances','activation':'GELU hidden, linear Gaussian mean/log-variance and readout','objective':'mean pixel MSE + (1/784) * mean Gaussian KL','optimizer':'Adam beta1=0.9 beta2=0.99 eps=1e-8','learningRate':RATE,'batchSize':BATCH,'updates':step,'trainingImages':len(x),'heldOutImages':len(test),'seed':7,'trainMSE':mse(x[:256]),'heldOutMSE':mse(test),'latentMin':z.min(0).tolist(),'latentMax':z.max(0).tolist(),'trainingSeconds':round(time.time()-started,2),'weightsFormat':'little-endian float32; each layer output-major weights then biases; latent layer has four outputs','labelsUsed':False,'sha256':hashlib.sha256(packed.tobytes()).hexdigest()}
    info.update(dataset='MNIST' if args.dataset=='mnist' else 'Fashion-MNIST',sources={name:hashlib.sha256((DATA/name).read_bytes()).hexdigest() for name in [f'{args.dataset}-train.png',f'{args.dataset}-test.png',f'{args.dataset}-meta.json']})
    (DATA/f'{args.dataset}-autoencoder.json').write_text(json.dumps(info,indent=2)+'\n');print(json.dumps(info),flush=True)
